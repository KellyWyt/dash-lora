...
#=================================================
#想通过这个实现，先通过一次前向传播，梯度大的top-2层分配单独的A，其余分配共享的A
...
def get_enable_layers(scores: pd.DataFrame, budget_portion: float):
    ...
    # Set minimum portion for ViT layers to at least 1/4 of the total budget,# ȷ���Ӿ������ٻ��25%��Ԥ�㣨��Ҫ����
    vit_min_budget = max(
    ...
    if llm_enable_layers_count > 0: # ѡ����Ҫ�Է�����ߵ�ǰK���㡣
        ...
        if len(llm_scores[llm_scores["score"] == llm_threshold]) > 1: #������������ͬʱ�����ѡ�����Ԥ�㡣��������ѡ��ͬһ���㣬���Ӷ����ԡ�
    ...
    # Ensure that the number of enabled layers matches the expected count,# ��֤ѡ��Ĳ�����ȷ
    assert (
...
for i in range(1, num_tasks + 1): #����ѧϰѭ��
    ...
    for layer in enable_layers:
        if layer not in cur_arch:
            cur_arch[layer] = [i] # �²㣬���ӵ�ǰ����
        else:
            cur_arch[layer].append(i) # ���в㣬׷������
