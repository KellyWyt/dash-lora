...
class VisualEncoder(nn.Module):
    ...
    def feature_select(self, image_forward_outs):
        features = []
        for lyr in self.select_layer_list:  # 第1-12层: 低层特征 → 边缘、纹理、颜色 (过于局部化) 第13层(-11): 中层特征 → 物体部件、局部关系 (恰到好处) 第24层(-1): 高层特征 → 全局场景、语义概念 (抽象概括)
            image_features = image_forward_outs.hidden_states[lyr]
            if self.select_feature == 'patch':
                image_features = image_features[:, 1:] # (10,256,124)
...
class VLProjector(nn.Module):
    def __init__(
        self,
        bert_ckpt_path = '/nfs1/outdated/WYT/models/bert-base-uncased',  # NOTE: replace this for load bert model
        hidden_size = 1024,
