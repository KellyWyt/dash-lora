...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # print('i am hree generate anagin!')
        all_things=model_kwargs["inputs_embeds"]
        if isinstance(all_things, list):
            model_kwargs["inputs_embeds"]=all_things[0]
            all_things=all_things[1:]
            # print('all modality')
        else:
            all_things=None
            # print('only text')
        ...
        # print(type(inputs_tensor_all),'after inputs_tensor')
        # print(len(inputs_tensor_all),'after inputs_tensor shape')
        ## inputs_tensor: list 5 embeddings & maskes

        # inputs_tensor=inputs_tensor_all[0]

        batch_size = inputs_tensor.shape[0]

        # print(model_input_name,'model_input_name')
        # print(type(model_kwargs['input_ids']))
        ...
        if not self.config.is_encoder_decoder:
            # print('this branch??? decoder only')
    ...
    def sample(
        self,
        input_ids: torch.LongTensor,
        modality_mask:List[torch.Tensor]=None,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        logits_warper: Optional[LogitsProcessorList] = None,
        max_length: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # prepare model inputs
            # print('mask',type(modality_mask))
            if(input_ids.size(1)==1):
                modality_mask.insert(0, input_ids) 
            # print('all inputs', type(modality_mask))
            # print(len(modality_mask))
            # print('come here again?')
            # print(input_ids.size())
            if(input_ids.size(1)>1):
                # print("[CHECK THIS S2] only text token")
                ### only text token
                model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            else:
                # print("[CHECK THIS S2] right forward token")
                model_inputs = self.prepare_inputs_for_generation(modality_mask, **model_kwargs)
