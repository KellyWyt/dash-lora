...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x,modality_mask):
        if self.config.pretraining_tp > 1:
            ...
            #down_proj = self.down_proj(self.act_fn(self.gate_proj(x,modality_mask)) * self.up_proj(x,modality_mask),modality_mask)

            a=self.gate_proj(x,modality_mask)
            b=self.act_fn(a) * self.up_proj(x,modality_mask)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        inputs_embeds_all: Optional[List[torch.FloatTensor]] = None,
