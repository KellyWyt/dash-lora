...
class LoraConfig(PeftConfig):
    ...
    # r: list = field(default=[16,16,16], metadata={"help": "Lora attention dimension"})
    lora_type: str = field(default=None, metadata={"help": "Lora type, moka or other lora"})
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, config, model): # LoraConfig, CasualLM
        ...
        self.forward = self.model.forward # �? self.model（底层大模型）的 forward 方法直接赋值给 LoraModel 实例�? forward 属�? /////////// LoraModel实例(input_ids) �? LoraModel.forward(input_ids)  # 这实际上是底层模型的forward方法�? UnifiedForCausalLM.forward(input_ids)
...
def init_lora_mem(lora_mem, k1):
    ...
    std = 0.14         # 标准差，经计算匹配kaiming_uniform的输出能�?
    
    # 前k1行：正向向量（均�?>0，符合文本模态极性）
    lora_mem.data[:k1] = torch.normal(mean=mean_pos, std=std, size=(k1, r))
    
    # 后k2行：负向向量（均�?<0，符合图像模态极性）
...
class LinearW(nn.Linear, LoraLayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A0"): # 初始�? A 矩阵（下投影矩阵�?
            for i in range(self.lora_num):
            ...
            init_weight = 0.1  # 初始权重值（可根据依赖程度调整）
            
            # �?0行：text mod �? 激活第1个专属区域（0 ~ m-1�?
            self.lora_trace.data[0, 0:m] = init_weight
            
            # �?1行：video mod �? 激活第2个专属区域（m ~ 2m-1�?
            self.lora_trace.data[1, m:2*m] = init_weight
            
            # �?2行：audio mod �? 激活第3个专属区域（2m ~ 3m-1�?
            self.lora_trace.data[2, 2*m:] = init_weight

            for i in range(2): # 初始�? B 矩阵（上投影矩阵�?
    ...
    def cross_attention_r(self, query, key_value, num):
        score = torch.matmul(query, key_value.transpose(-2, -1))/ math.sqrt(num)  # shape: (1, token_num, question_length)
        score = torch.softmax(score, dim=-1)
        output = torch.matmul(score, key_value)  # shape: (1, token_num, 4)
        return output + query
    ...
    def cross_attention(self, query, key_value):
        ...
        out = torch.einsum('bnr, brs -> bns', q, kv)  # 先计�? Q与kv的乘�?
        out = out
    ...
    def forward(self, x: torch.Tensor, modality_mask:List[torch.Tensor]=None):
        ...
        if(('test' in self.loramethod) &(x.size(1)==1) ):
            ...
            input_b=output_a
            new_loraA = self.act_memory_soft(x=input_b, mode=0)*self.scaling[0]  # [b, n, r]
        ...
        if(('test' in self.loramethod) & (x.size(1)!=1)):
            ...
            new_loraAAA = []
            for i in range(self.lora_num):
                input_b = getattr(self, f"lora_A{i}")(self.lora_dropout(only_inputs[i]))*self.scaling[0]
                new_loraA = self.act_memory_soft(x=input_b, mode=i)*self.scaling[0]
            ...
            video_token=new_loraAAA[1]*video_mask   # only needed available
            video_token_base=output_a[1]
            ### audio_token: cross attention per sample
            audio_token=new_loraAAA[2]*audio_mask
            ...
            for i in range(question_token_base.size(0)):
                mem_v=video_token[i:i+1,:,:]
                mem_a=audio_token[i:i+1,:,:]
                base_v=video_token_base[i:i+1,:,:].mean(dim=1, keepdims=True)
                base_a=audio_token_base[i:i+1,:,:].mean(dim=1, keepdims=True)
                ## get question tokens
                indices = torch.where(question_mask[i,:,:] == 1)[0]
                base_t=question_token_base[i:i+1,indices[0]:indices[-1]+1,:]

                N_t=torch.sum(question_mask[i,:,:])
                supp_v = self.cross_attention_r(query=mem_v, key_value=torch.cat([base_t, base_a], dim=1), num=N_t+1)  # shape: (1, token_num, 4)
                supp_a = self.cross_attention_r(query=mem_a, key_value=torch.cat([base_t, base_v], dim=1), num=N_t+1)  # shape: (1, token_num, 4)
            ...
            input_b1 = [new_loraAAA[0]*text_mask, new_video, new_audio]
            input_b1 = sum(input_b1)

            output_b=getattr(self, f"lora_B0")(input_b) + getattr(self, f"lora_B1")(input_b1)
        ...
        if('train' in self.loramethod):
            ...
            ### video_token: cross attention per sample
            video_token=new_loraAAA[1]*video_mask
            video_token_base=output_a[1]
            ### audio_token: cross attention per sample
            audio_token=new_loraAAA[2]*audio_mask
            audio_token_base=output_a[2]
...
class Linear(nn.Linear, LoraLayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A0"): # 初始�? A 矩阵（下投影矩阵�?
            ...
            for i in range(1): # 初始�? B 矩阵（上投影矩阵�?
                nn.init.zeros_(getattr(self, f"lora_B{i}").weight)
    ...
    def forward(self, x: torch.Tensor, modality_mask:List[torch.Tensor]=None):
        ...
        ## infer first forward
        if(('test' in self.loramethod) &(x.size(1)!=1) ):
            # print(f"[CHECK THIS] modality mask length: {len(modality_mask)}")
