    ...
    **Attributes**:
    ...
    def _create_shared_A_matrices(self,target_modules): #NOTE
        ...
        for proj_type in target_modules:
            ...
            # Strict cross-modal baseline: non-exclusive modality branches share
            # the exact same rank-r Parameter; scored exclusives stay unchanged.
            type_params = {}
            cross_modal_param = nn.Parameter(torch.empty(rr[0], in_features))
            nn.init.kaiming_uniform_(cross_modal_param, a=math.sqrt(5))
            for i in range(min(self.peft_config.lora_nums, len(rr))):
                type_params[f'shared_A{i}'] = cross_modal_param
