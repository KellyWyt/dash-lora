...
def train(attn_implementation=None):
    # print("WANDB_PROJECT in script: ", os.environ.get("WANDB_PROJECT", "NOT FOUND"))
    # print('lalala')
    # return
    global local_rank
    set_seed(42)
    ...
    if model_args.llm_name == 'llama':
        ...
        model = UnifiedForCausalLM.from_pretrained(
            ...
            # low_cpu_mem_usage=True   # NOTE: lower memory used, if memory is not enough, set this to True
        )
    ...
    if training_args.lora_enable:
        ### hyper lora
        from peft_hyper_dash_crossmodal_strict import LoraConfig,get_peft_model
    ...
    for name, param in model.named_parameters():
        # print(f'{name}: {param.device}') # 判断是否在GPU�?
        require_grad = False
