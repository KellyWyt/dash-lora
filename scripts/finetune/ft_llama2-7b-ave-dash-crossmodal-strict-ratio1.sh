...
RUN_NAME=llama2-7b-ave-dash-crossmodal-strict-ratio1
OUTPUT_LOG=results/log/llama2/dash-crossmodal-strict-ratio1.log
export CUDA_VISIBLE_DEVICES=0,1,2,3
export TOKENIZERS_PARALLELISM=true
mkdir -p "$(dirname "$OUTPUT_LOG")"

torchrun --nproc_per_node "$NPROC_PER_NODE" --master_port "$MASTER_PORT" \
    scripts/finetune/finetune_dash_crossmodal_strict.py \
    --deepspeed deepspeed/stage1-offload.json \
    --llm_name llama --model_name_or_path "$llama_ckpt_path" \
    --loramethod train \
    --dash_lora_safe_importance True \
    --reserved_modality None --exp_desc dash-lora-crossmodal-strict-ratio1 \
