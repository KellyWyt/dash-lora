...
def inference_avqa(dataset,collator,dataloader,ckpt_dir,model,tokenizer,num, seed: int, mode: str):
    ...
    subset_dataloader = DataLoader(subset, batch_size=1, shuffle=False,collate_fn=collator,drop_last=False,num_workers=4,pin_memory=True,prefetch_factor=2)
...
def train(attn_implementation=None):
    ...
    extra_parser.add_argument("--dash_ablation_mode", type=str, default="energy")
    extra_parser.add_argument("--dash_ablation_seed", type=int, default=123)
    extra_parser.add_argument("--dash_ablation_topology_path", type=str, default=None)
    ...
    if extra_args.dash_ablation_topology_path is None:
        candidate = join(ckpt_dir, "dash_ablation_topology.json")
        parent_candidate = join(os.path.dirname(ckpt_dir), "dash_ablation_topology.json")
        extra_args.dash_ablation_topology_path = candidate if exists(candidate) else parent_candidate
    ...
    if training_args.lora_enable:
        ...
        from peft_hyper_dash_ablation.tuners.dash_ablation_importance import normalize_mode
        ...
        peft_config = LoraConfig(
            ...
            **({"safe_importance": True} if training_args.dash_lora_safe_importance else {}),
            ...
            dash_ablation_mode=normalize_mode(extra_args.dash_ablation_mode),
            dash_ablation_seed=extra_args.dash_ablation_seed,
            dash_ablation_topology_path=extra_args.dash_ablation_topology_path,
        )
