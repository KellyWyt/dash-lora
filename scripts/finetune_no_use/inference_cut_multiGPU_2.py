...
def inference_avqa(dataset,collator,dataloader,ckpt_dir,model,tokenizer,num, seed: int, mode: str):
    ...
    save_dir = join(ckpt_dir,f'inference_avqa/bs6_tf32_mt150_seed{seed}_{mode}')
    ...
    subset_dataloader = DataLoader(subset, batch_size=2, shuffle=False,collate_fn=collator,drop_last=False)
...
def train(attn_implementation=None):
    ...
    # ckpt_path = join(ckpt_dir,'exp_B_prune_low_score.bin') #NOTE
    ...
    # model.cuda() # <--- 问题在这里，所有人都挤到默认显卡去�?
    # ===【修改开始�?===
    # 根据当前进程�? local_rank，强制绑定对应的显卡
