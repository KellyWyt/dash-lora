...
# from .importance_compute import HierarchicalImportanceAnalyzer #NOTE
from .importance_compute_multi_GPU_only_norm_ratio_3 import HierarchicalImportanceAnalyzer
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _analyze_and_setup_hierarchical_lora(self): #NOTE
        ...
        # NOTE Returns a DICT now: {'text_exclusive': [], 'others_exclusive': [], 'scores': {}}
        analysis_result = analyzer.analyze_model_only_target_modules(
            self.model, self.peft_config.target_modules, target_module_keys, 
            top_k=self.peft_config.top_k_layers # This is the TOTAL budget
        )

         # ===【新增代码开始】===
        # 分析完了，analyzer 没用了，赶紧删掉并清理显存 #TODO
        del analyzer
        import gc
        gc.collect()
        ...
        for key in key_list:
            ...
            if target_module_found: # here
                ...
                if isinstance(target, torch.nn.Linear) and self.peft_config.enable_lora is None:
                    new_module = Linear_eff(target.in_features, target.out_features, bias=bias, **kwargs)
...
class Linear_eff(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        text_selection_ratio: float = 0.9, # NOTE [New] 新增：文本筛选比例，例如0.5表示只保留前50%重要的文本token
        **kwargs,
    ):
        ...
        # NOTE [CRITICAL] 保存独立性状态，供 reset_parameters 使用
        self.is_text_independent = is_text_independent
        self.is_others_independent = is_others_independent

        self.text_selection_ratio = text_selection_ratio # NOTE [New]
    ...
    def forward(self, x: torch.Tensor, modality_mask:List[torch.Tensor]=None):
        ...
        if(('test' in self.loramethod) &(x.size(1)!=1) ):
            ...
            # NOTE [New] 初始化文本重要性累加器 (Batch, Seq_Len)
            # 用于记录每个文本Token被Visual/Audio关注的总程度
            total_text_importance = torch.zeros(x.size(0), x.size(1)).to(x.device)
            ...
            for i in range(question_token.size(0)):
                ...
                # NOTE [New] 收集 Video 对 Text 的关注度
                # 逻辑：对于每个文本词，找出最关注它的那个视频帧的权重 (Max Pooling)
                # 这样可以捕捉到该词是否对视频的任意部分有意义
                vid_text_imp, _ = score.max(dim=1) # [1, N_valid_text]
                
                # 将重要性累加回全局累加器 (对应 indices 位置)
                total_text_importance[i, indices[0]:indices[-1]+1] += vid_text_imp.squeeze(0)
            ...
            for i in range(question_token.size(0)):
                ...
                # NOTE [New] 收集 Audio 对 Text 的关注度
                aud_text_imp, _ = score.max(dim=1) # Max over audio frames
                total_text_importance[i, indices[0]:indices[-1]+1] += aud_text_imp.squeeze(0)
            ...
            # =================== [New] 生成 Text Mask 并应用 ===================
            
            # 此时 total_text_importance 包含了每个文本 Token 的总关注度
            # 我们根据 text_selection_ratio 生成 Mask
            
            final_text_mask = torch.zeros_like(output_a[0]) # [Batch, Seq, Dim]
            ...
            for i in range(question_token.size(0)):
                ...
                # 获取该样本有效文本的关注度
                valid_imp = total_text_importance[i, indices[0]:indices[-1]+1] # [N_valid_text]
                
                # 确定 Top-K 阈值
                k = int(valid_imp.size(0) * self.text_selection_ratio)
                if k > 0:
                    topk_val, _ = torch.topk(valid_imp, k)
                    threshold = topk_val[-1]
                    # 生成 Mask: 大于阈值的为1，否则为0
                    # 注意：这是 [Seq_Len] 维度的 Mask
                    seq_mask = (valid_imp >= threshold).float()
                else:
                    seq_mask = torch.zeros_like(valid_imp)
                
                # 将 Seq Mask 扩展到 Feature 维度 [Seq, Dim] 并填回
                # 这里的逻辑是：如果这个词不重要，它的整个 Embedding 向量都被 Mask 掉
                final_text_mask[i, indices[0]:indices[-1]+1, :] = seq_mask.unsqueeze(-1)

            # [New] 应用 Mask 到 Text Feature (output_a[0])
            # 这就是你要的 "只微调这些更关注的 token"
            filtered_text_features = output_a[0] * final_text_mask

            # =================== 合并与输出 ===================
            
            # 使用筛选后的 text features 组合 input_b
            input_b = [filtered_text_features, new_video, new_audio]
