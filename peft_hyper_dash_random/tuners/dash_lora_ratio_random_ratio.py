...
from .importance_compute_multi_GPU_only_norm_ratio_3_random_ratio import HierarchicalImportanceAnalyzer
    ...
    **Attributes**:
    ...
    def _analyze_and_setup_hierarchical_lora(self): #NOTE
        ...
        analysis_result = analyzer.analyze_model_only_target_modules(
            ...
            top_k=self.peft_config.top_k_layers, # 56
        ...
        self.peft_config.video_important_modules = analysis_result['video_exclusive']
        self.peft_config.audio_important_modules = analysis_result['text_exclusive']
    ...
    def _find_and_replace_hierarchical(self,target_module_keys): #NOTE dash-lora-compare
        ...
        video_list = set(self.peft_config.video_important_modules) #NOTE dash-lora-compare
        audio_list = set(self.peft_config.audio_important_modules)
        ...
        for key in target_module_keys:
            ...
            is_video_independent = key in video_list #NOTE dash-lora-compare
            is_audio_independent = key in audio_list
            ...
            self._replace_single_module_asymmetric(
                key, kwargs, 
                is_text_independent, 
                is_video_independent,
                is_audio_independent, #NOTE dash-lora-compare
                shared_params
            )
    ...
    def _replace_single_module_asymmetric(self, key, kwargs, is_text_independent,is_video_independent, is_audio_independent, shared_A_matrices): #NOTE dash-lora-compare
        try:
            ...
            if isinstance(target, torch.nn.Linear):
                new_module = Linear(
                    ...
                    is_video_independent=is_video_independent,
                    is_audio_independent=is_audio_independent, #NOTE dash-lora-compare
...
class Linear_1(nn.Linear, LoraLayer): #NOTE dash-lora
...
class Linear(nn.Linear, LoraLayer): #NOTE dash-lora
    # Lora implemented in a dense layer
    def __init__(
        self,
        in_features: int,
        ...
        shared_A_matrices: dict = None, # TODO
        is_text_independent: bool = False,   # A0 exclusive?
        is_video_independent: bool = False, # A1 exclusive? #NOTE dash-lora-compare
        is_audio_independent: bool = False, # A2 exclusive?
        # is_others_independent: bool = False, # A1, A2 exclusive?
        ratio:int = 1,
        **kwargs,
    ):
        ...
        self.is_text_independent = is_text_independent
        self.is_video_independent = is_video_independent #NOTE dash-lora-compare
        self.is_audio_independent = is_audio_independent
        # self.is_others_independent = is_others_independent
        ...
        if rr[0] > 0:
            ...
            #             setattr(self, f"lora_A{i}", nn.Linear(in_features, rr[i], bias=False))
            # === A1: Video Modalities ===
            for i in range(1, self.lora_num-1): #NOTE dash-lora-compare
                if is_video_independent:
                    setattr(self, f"lora_A{i}", nn.Linear(in_features, rr[i], bias=False))
                else:
                    if shared_A_matrices and f'shared_A{i}' in shared_A_matrices:
                        l_a = nn.Linear(in_features, rr[i], bias=False)
                        ...
                        setattr(self, f"lora_A{i}", nn.Linear(in_features, rr[i], bias=False))
            # === A2...: Audio Modalities ===
            for i in range(2, self.lora_num): #NOTE dash-lora-compare
                if is_audio_independent:
                    setattr(self, f"lora_A{i}", nn.Linear(in_features, rr[i], bias=False))
                else:
                    if shared_A_matrices and f'shared_A{i}' in shared_A_matrices:
                        l_a = nn.Linear(in_features, rr[i], bias=False)
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A0"):
            ...
            for i in range(1, self.lora_num-1):
                if hasattr(self, f"lora_A{i}"):
                    if self.is_video_independent: #NOTE dash-lora compare-random-ratio
                         nn.init.kaiming_uniform_(getattr(self, f"lora_A{i}").weight, a=math.sqrt(5))
            for i in range(2, self.lora_num):
                if hasattr(self, f"lora_A{i}"): #NOTE dash-lora compare-random-ratio
                    if self.is_audio_independent:
                         nn.init.kaiming_uniform_(getattr(self, f"lora_A{i}").weight, a=math.sqrt(5))
            for i in range(1):
                if hasattr(self, f"lora_B{i}"):
                    nn.init.zeros_(getattr(self, f"lora_B{i}").weight)
