...
class HierarchicalImportanceAnalyzer:
    ...
    def _analyze_multi_gpu(self, model, projection_types, target_module_keys, top_k,ratio):
        ...
        return self._select_asymmetric_split_random(all_scores, top_k, ratio)
    ...
    def analyze_single_gpu_safe(self, model, target_module_keys, top_k, ratio):
        """Analyze one rank in FP32 and fail before broadcasting an incomplete topology."""
        ...
        if len(all_raw) != len(target_module_keys):
            raise RuntimeError(
                f"Importance analysis collected {len(all_raw)}/{len(target_module_keys)} modules"
            )
        all_scores = self._compute_final_scores(all_raw)
        return self._select_asymmetric_split_random(all_scores, top_k, ratio)
