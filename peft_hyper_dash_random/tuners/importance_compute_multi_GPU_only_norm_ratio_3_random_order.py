...
class HierarchicalImportanceAnalyzer:
    ...
    def _select_asymmetric_split_dynamic(self, all_scores, total_budget,ratio): # NOTE dash-lora but add print
    ...
    # NOTE: Dynamic ratio computation — replaces the manual `ratio` hyperparameter.
    # Instead of a fixed text:others budget ratio set by the user, this method derives
    # the ratio automatically from the raw norm distribution of all target modules.
    # Intuition: layers with higher norm are more "active" and need more exclusive
    # adaptation capacity (larger text budget); layers with lower norm are more stable
    # and can share A matrices across modalities.
    # Concretely: split all module norms at the median, compute mean of each half,
    # and use their ratio as the text:others budget ratio, clipped to [clip_min, clip_max].
    # NOTE: Limitation — norm magnitude reflects weight scale, NOT modality sensitivity.
    # This is a proxy heuristic. A more principled approach would use per-modality
    # activation norms from a forward pass (see future work).
    def _compute_dynamic_ratio(self, all_raw_metrics, clip_min=1.0, clip_max=8.0):
        norms = np.array([m['norm'] for m in all_raw_metrics.values()])
        if len(norms) < 2:
            return 3.0  # NOTE: fallback to default ratio when too few modules
        median = np.median(norms)
        mean_high = norms[norms >= median].mean()
        mean_low  = norms[norms <  median].mean()
        if mean_low < 1e-6:
            return clip_max  # NOTE: avoid division by zero; saturate to max ratio
        dynamic_ratio = float(np.clip(mean_high / mean_low, clip_min, clip_max))
        print(f"[DynamicRatio] mean_high={mean_high:.2f}, mean_low={mean_low:.2f} → ratio={dynamic_ratio:.3f}")
    ...
    def _select_asymmetric_split_random_order(self, all_scores, total_budget, ratio):
        ...
        # rng = random.Random(42) #NOTE seed
        ...
        print(f"Text Exclusive Modules:")
        for m in text_exclusive:
            print(f"  [TEXT]   {m}")
        print(f"Others Exclusive Modules:")
        for m in others_exclusive:
            print(f"  [OTHERS] {m}")
    ...
    def _analyze_multi_gpu(self, model, projection_types, target_module_keys, top_k,ratio):
        ...
        # NOTE: Replace fixed `ratio` hyperparameter with a data-driven value derived
        # from the norm distribution of all target modules. The incoming `ratio` argument
        # is intentionally ignored here; dynamic_ratio is computed from all_raw instead.
        # dynamic_ratio = self._compute_dynamic_ratio(all_raw)
        return self._select_asymmetric_split_random_order(all_scores, top_k, ratio)
