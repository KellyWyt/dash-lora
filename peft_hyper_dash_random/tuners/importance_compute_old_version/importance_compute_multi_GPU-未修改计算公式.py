...
class HierarchicalImportanceAnalyzer:
    ...
    def _select_top_k_by_type_statistic(self, all_scores, type_scores, top_k):
        #TODO '''statistic allocate'''
        """选择top_k模块，排除31层，但最终返回包含31层所有模块"""
        # 要排除的层号
        ...
        non_31_scores = {}    # 非31层的分数
        non_31_type_scores = {}  # 非31层按类型分组的分数
        ...
        if top_k == 50:
            ...
            # 对于其他top_k值，使用动态比例计算
            scale_factor = top_k / 50.0  # 以50为基准
            recommended_quotas = {
        ...
        if current_total > top_k:
            ...
            while current_total > top_k:
                ...
                if adjusted_quotas[max_type] > 1:  # 至少保留1个
    ...
    def _select_top_k_by_type(self, all_scores, type_scores, top_k):
        """
        动态选择 top_k 模块 (改进版):
        1. 自动检测最后一层并强制保留（Last Layer Protection）。
        2. 剩余名额在所有其他层中通过“全局分数竞争”分配，不再人为设定类型配额。
        """
        ...
        # 自动检测最大的层号（Last Layer）
        max_layer_num = -1
        for name in all_scores.keys():
            num = extract_layer_num(name)
            if num > max_layer_num:
                max_layer_num = num
        
        exclude_layer_num = max_layer_num  # 动态设定，不再是硬编码的 31
        # print(f"Detected last layer: {exclude_layer_num}")

        # --- 2. 分离候选池 ---

        last_layer_modules = []   # 最后一层的模块 (必须保留)
        candidate_pool = []       # 其他层的候选池 (参与竞争)
        
        for name, score in all_scores.items():
            layer_num = extract_layer_num(name)
            if layer_num == exclude_layer_num:
                last_layer_modules.append((name, score))
            elif layer_num != -1: # 排除无法识别层号的异常模块
                candidate_pool.append((name, score))

        # --- 3. 名额分配逻辑 ---

        num_last_layer = len(last_layer_modules)
        
        # 计算留给其他层的名额
        # 如果 top_k 小于最后一层模块数，优先满足最后一层
        if top_k <= num_last_layer:
            # 这种情况极少见，但为了代码健壮性
            print(f"Warning: top_k ({top_k}) is smaller than last layer modules ({num_last_layer}).")
            # 在最后一层内部卷
            last_layer_modules.sort(key=lambda x: x[1], reverse=True)
            selected_last = last_layer_modules[:top_k]
            selected_candidates = []
        else:
            # 正常情况：保留全部最后一层，剩余名额给其他层
            selected_last = last_layer_modules
            slots_remaining = top_k - num_last_layer
            
            # --- 核心修改：全局分数排序 ---
            # 不再区分 o_proj/q_proj 的配额，直接按分数排序
            candidate_pool.sort(key=lambda x: x[1], reverse=True)
            selected_candidates = candidate_pool[:slots_remaining]

        # --- 4. 构建最终结果 ---
        
        top_k_layers = [x[0] for x in selected_last] + [x[0] for x in selected_candidates]
        final_scores = {**dict(selected_last), **dict(selected_candidates)}

        # --- 5. 打印统计信息 (Visualizing the Dynamic Result) ---
        
        print(f"\n{'='*80}")
        print(f"DYNAMIC ALLOCATION (top_k={top_k}, Last Layer={exclude_layer_num})")
        print(f"{'='*80}")

        # 统计选中的非最后一层模块的类型分布（看看数据到底觉得谁重要）
        ...
        for name, score in selected_candidates:
            ...
            for p_type in ['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'down_proj', 'up_proj']:
                if p_type in name:
                    selected_type_counts[p_type] = selected_type_counts.get(p_type, 0) + 1
                    break
        
        # 统计最后一层的类型
        last_layer_type_counts = {}
        for name, score in selected_last:
            for p_type in ['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'down_proj', 'up_proj']:
                if p_type in name:
                    last_layer_type_counts[p_type] = last_layer_type_counts.get(p_type, 0) + 1
                    break

        print(f"\n1. Last Layer Protection (Layer {exclude_layer_num}):")
        print(f"   Selected {len(selected_last)} modules.")
        
        print(f"\n2. Global Competition Results (Non-Last Layer):")
        print(f"   Slots Available: {len(selected_candidates)} (Top-{top_k} - {len(selected_last)})")
        print(f"   Selection Breakdown by Type (Data-Driven):")
        
        # 按选中数量排序打印
        sorted_counts = sorted(selected_type_counts.items(), key=lambda x: x[1], reverse=True)
        print(f"   {'Type':12s} {'Count':6s} {'% of Selection'}")
        print(f"   {'-'*40}")
        
        if not selected_candidates:
            print(f"   (No modules selected from other layers)")
        
        for p_type, count in sorted_counts:
            pct = (count / len(selected_candidates) * 100)
            print(f"   {p_type:12s} {count:6d} {pct:8.1f}%")

        # 打印详细列表的前几个和后几个
        print(f"\n   Top 5 Selected (Highest Scores):")
        for i, (name, score) in enumerate(selected_candidates[:5]):
            parts = name.split('.')
            simple_name = f"...{parts[-3]}.{parts[-2]}.{parts[-1]}" if len(parts)>3 else name
            print(f"     {i+1}. {simple_name:40s}: {score:.4f}")

        if len(selected_candidates) > 5:
            print(f"     ...")
            print(f"   Bottom 3 Selected (Lowest Scores within Cutoff):")
            for i, (name, score) in enumerate(selected_candidates[-3:]):
                parts = name.split('.')
                simple_name = f"...{parts[-3]}.{parts[-2]}.{parts[-1]}" if len(parts)>3 else name
                print(f"     {len(selected_candidates)-2+i}. {simple_name:40s}: {score:.4f}")

        print(f"{'='*80}")
        print(f"Total Modules Selected: {len(top_k_layers)}")
        print(f"{'='*80}")
