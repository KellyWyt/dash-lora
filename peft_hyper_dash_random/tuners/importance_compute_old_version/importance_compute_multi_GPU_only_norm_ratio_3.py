...
#         Total Slots Budget = total_budget * 2 (例如 56 * 2 = 112)
#         Constraint: N_text + N_video + N_audio = Total Slots
#         Assume: N_video = N_audio = N_others
#         Random Ratio: N_text / N_others = ratio (randomly sampled)
#         """
        
        # 1. 排序：分数从低到�? (Low Score = High Norm = Rank 1)
        sorted_items = sorted(all_scores.items(), key=lambda x: x[1])
        all_modules_sorted = [x[0] for x in sorted_items]
        max_layers = len(all_modules_sorted)
        
        # 2. 定义总名额池 (Actual Budget)
        # 原逻辑�? 56 + 28 + 28 = 112，所以这里设为传入budget�?2�?
        actual_budget = total_budget * 2 
        
        # 3. 生成随机比例 (Random Ratio)
        # 设定一个合理的范围，例�? [1.5, 3.0]�?
        # 2.0 是您目前的最佳实�? (2:1)�?
        # 1.5 代表 Text:Others = 3:2 (Text稍微少一�?)
        # 3.0 代表 Text:Others = 3:1 (Text占绝对主�?)
        ratio_ = ratio
        
        # 4. 解方程计算截断点
        # Equation: N_text + 2 * N_others = actual_budget
        # Substitute N_text = ratio * N_others
        # => N_others * (ratio + 2) = actual_budget
        ...
        limit_others = min(max(1, limit_others), limit_text) # 确保 Others �? Text 的子�?(通常 Text 覆盖更广)
