...
# from .importance_compute_multi_GPU_only_norm_ratio_3_copy import HierarchicalImportanceAnalyzer #NOTE
# from .importance_compute_multi_GPU_only_norm_ratio_3_random_order import HierarchicalImportanceAnalyzer
from .importance_compute_multi_GPU_only_norm_ratio_3_copy_copy import HierarchicalImportanceAnalyzer #NOTE
    ...
    **Attributes**:
    ...
    def _analyze_and_setup_hierarchical_lora(self): #NOTE
        ...
        if safe_importance:
            ...
            # Historical behavior for existing Llama2 and Qwen2 experiments.
            analyzer = HierarchicalImportanceAnalyzer()
        ...
        # Convert Dash-LoRA's fine-grained exclusive-slot budget into the
        # closest possible number of complete transformer layers.  A Llama2
        # layer contains 7 target projections and 3 modality-specific A slots.
        # For ratio 1 / top_k 56, Dash has 37 + 2*37 = 111 exclusive slots;
        # round(111 / (7*3)) = 5 whole exclusive layers.  This gives 126 unique
        # A matrices versus Dash's 132 (the closest attainable whole-layer
        # count; error 6).
        ...
        for module_name, score in scores.items():
            ...
            layer_scores[layer_id] = layer_scores.get(layer_id, 0.0) + float(score)
            layer_counts[layer_id] = layer_counts.get(layer_id, 0) + 1
        layer_scores = {
            layer_id: total / layer_counts[layer_id]
            for layer_id, total in layer_scores.items()
        }

        target_projections = len(self.peft_config.target_modules)
        modality_count = self.peft_config.lora_nums
        dash_exclusive_slots = (
            len(analysis_result['text_exclusive'])
            + (modality_count - 1) * len(analysis_result['others_exclusive'])
        )
        slots_per_whole_layer = target_projections * modality_count
        exclusive_layer_count = int(
            dash_exclusive_slots / slots_per_whole_layer + 0.5
        )
        exclusive_layer_ids = {
            layer_id for layer_id, _ in sorted(
                layer_scores.items(), key=lambda item: item[1]
            )[:exclusive_layer_count]
        }
        whole_layer_exclusive = [
            key for key in target_module_keys
            if any(f'.layers.{layer_id}.' in key for layer_id in exclusive_layer_ids)
        ]

        self.peft_config.text_important_modules = whole_layer_exclusive
        self.peft_config.others_important_modules = whole_layer_exclusive
        self.peft_config.layerwise_exclusive_layer_ids = sorted(exclusive_layer_ids)
        self.peft_config.dash_reference_unique_a = (
            target_projections * modality_count + dash_exclusive_slots
        )
        self.peft_config.layerwise_unique_a = (
            target_projections * modality_count
            + exclusive_layer_count * slots_per_whole_layer
        )
        print(
            "Layerwise count match: "
            f"exclusive_layers={sorted(exclusive_layer_ids)}, "
            f"unique_A={self.peft_config.layerwise_unique_a}, "
            f"dash_reference={self.peft_config.dash_reference_unique_a}"
        )
        ...
        self.peft_config.shared_modules = list(
            set(target_module_keys) - set(whole_layer_exclusive)
        )
    ...
    def _create_shared_A_matrices(self,target_modules): #NOTE
        ...
        for proj_type in target_modules:
            ...
            if in_features is None and safe_importance:
                ...
                for module_name, module in self.model.named_modules():
                    if module_name.endswith(proj_type) and hasattr(module, "in_features"):
